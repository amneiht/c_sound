...
int playback_callback(snd_pcm_sframes_t nframes) {
	...
	/* ... fill buf with data ... */
	nframes = snd_pcm_readi(cap_handle, buf, 4096);
...
int mainInnet(int argc, char *argv1[]) {
	...
	while (1) {
		...
		if ((frames_to_deliver = snd_pcm_avail_update(playback_handle)) < 0) {
			if (frames_to_deliver == -EPIPE) {
				...
				fprintf(stderr,
						"unknown ALSA avail update return value (%ld)\n",
						frames_to_deliver);
				break;
			}
		}
