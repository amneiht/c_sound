...
void info(char *dev_name, snd_pcm_stream_t stream) {
	...
	if ((err = snd_pcm_hw_params_get_period_size_min(hw_params, &frames, &dir))
			< 0) {
		fprintf(stderr, "cannot get min period size (%s)\n", snd_strerror(err));
		exit(1);
	}
	printf("min period size in frames %ld\n", frames);
	if ((err = snd_pcm_hw_params_get_period_size_max(hw_params, &frames, &dir))
			< 0) {
		fprintf(stderr, "cannot get max period size (%s)\n", snd_strerror(err));
		exit(1);
	}
	printf("max period size in frames %ld\n", frames);
	...
	printf("min buffer size in frames %ld\n", frames);
...
//	}
	...
	printf("*********** PLAYBACK ***********\n");
	info(device, SND_PCM_STREAM_PLAYBACK);
	exit(0);
}
